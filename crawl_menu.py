...
class Config:
    """爬蟲配置"""
    CDP_PORT = int(os.getenv("CRAWLER_CDP_PORT", "9222"))
    ...
    ENABLE_MANUAL_ASSIST = os.getenv("CRAWLER_MANUAL_ASSIST", "false").lower() in {
        "1", "true", "yes", "on"
    }
    PROFILE_DIR = Path(__file__).resolve().parent / ".chrome_debug_profile"
    DEBUG_DIR = Path(__file__).resolve().parent / "logs" / "crawler"
...
def start_chrome_debug_mode():
    ...
    try:
        ...
        if not Path(Config.CHROME_PATH).is_file():
            print(f"  [FAIL] 找不到 Chrome: {Config.CHROME_PATH}")
            return False
...
async def find_and_click_menu_button(page) -> bool:
    ...
    try:
        ...
        debug_path = Config.DEBUG_DIR / "debug_no_menu_button.png"
        await page.screenshot(path=str(debug_path))
        print(f"[SCREENSHOT] 已儲存除錯截圖: {debug_path}")
    except:
        pass
...
def extract_menu_items_from_page_text(text: str) -> List[MenuItem]:
    """Extract item/price pairs from a public linked menu page's visible text."""
    lines = [re.sub(r"\s+", " ", line).strip() for line in (text or "").splitlines()]
    lines = [line for line in lines if line]
    price_pattern = re.compile(r"^(?:NT\$|NTD\s*|\$)\s*(\d{1,5})(?:\.\d+)?(?:\s*起)?$")
    rejected_names = {
        "外送", "自取", "菜單", "全日菜單", "熱門餐點", "商店資訊", "常見問題",
        "登入", "註冊", "熱門類別", "熱門城市", "熱門地區", "價格",
    }
    items: List[MenuItem] = []
    seen = set()
    for index in range(1, len(lines)):
        ...
        name = lines[index - 1]
        if (
            name in rejected_names
            or name in seen
            or len(name) < 2
            or len(name) > 80
            or name.startswith(("排名第", "星期", "©"))
            or re.fullmatch(r"[\d\s.%()+-]+", name)
        ):
            continue
        items.append(MenuItem(name=name, price=match.group(1)))
        seen.add(name)
    return items


def _unwrap_google_result_url(href: str) -> str:
    if not href:
        return ""
    parsed = urlparse(href)
    if parsed.netloc.endswith("google.com") and parsed.path == "/url":
        href = (parse_qs(parsed.query).get("q") or parse_qs(parsed.query).get("url") or [""])[0]
        parsed = urlparse(href)
    host = parsed.netloc.lower().split(":", 1)[0]
    if not parsed.scheme.startswith("http"):
        return ""
    if not any(host == domain or host.endswith(f".{domain}") for domain in LINKED_MENU_DOMAINS):
        return ""
    return href


async def parse_menu_from_linked_pages(page, restaurant_name: str) -> Restaurant:
    """Follow public menu links surfaced by Google and parse visible item/price pairs."""
    print("\n[Fallback] 解析 Google 搜尋結果連結的公開菜單")
    candidates: List[str] = []
    seen = set()
    for query in (f"{restaurant_name} 菜單 價格", f"{restaurant_name} Uber Eats"):
        try:
            await page.goto(
            ...
            for href in hrefs:
                normalized = _unwrap_google_result_url(str(href))
            ...
            print(f"  [Linked] 搜尋失敗: {exc}")

    for url in candidates[:4]:
        print(f"  [Linked] 嘗試公開菜單: {url[:140]}")
        try:
            await page.goto(url, wait_until="domcontentloaded", timeout=45000)
            await page.wait_for_timeout(3500)
            body_text = await page.locator("body").inner_text(timeout=10000)
            items = extract_menu_items_from_page_text(body_text)
            if is_usable_menu_result(items):
                print(f"  [Linked] 成功解析 {len(items)} 道菜")
                return Restaurant(name=restaurant_name, menu_items=items)
            print(f"  [Linked] 只解析到 {len(items)} 道菜，繼續嘗試")
        except Exception as exc:
            print(f"  [Linked] 解析失敗: {exc}")
    return Restaurant(name=restaurant_name, menu_items=[])
...
async def crawl_google_menu(restaurant_name: str) -> Restaurant:
    ...
    async with async_playwright() as p:
        try:
            ...
            search_url = f"https://www.google.com/search?q={quote_plus(restaurant_name)}"
            ...
            if not click_success:
                print("\n" + "="*70)
                print("[WARNING] 找不到 Google 菜單按鈕，改查公開菜單連結")
                print("="*70)
                linked_result = await parse_menu_from_linked_pages(page, restaurant_name)
                if linked_result.menu_items:
                    return linked_result
                print("[Fallback] 公開菜單連結無資料，改從搜尋結果找菜單圖片")
            ...
            if not menu_loaded:
                ...
                print("="*70)
                print("[Fallback] 改查公開菜單連結")
                linked_result = await parse_menu_from_linked_pages(page, restaurant_name)
                if linked_result.menu_items:
                    return linked_result
                print("[Fallback] 公開菜單連結無資料，改從 Google 搜尋結果尋找菜單圖片")
            ...
            if not restaurant.menu_items:
                print("[Fallback] 文字菜單抽取為空，改查公開菜單連結")
                linked_result = await parse_menu_from_linked_pages(page, restaurant_name)
                if linked_result.menu_items:
                    return linked_result
                print("[Fallback] 公開菜單連結無資料，改從 Google 搜尋結果尋找菜單圖片")
