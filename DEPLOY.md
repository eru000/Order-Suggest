...
| `VISION_MODEL` | `ornith-35b` | 菜單照片辨識 |
| `VISION_VERIFY_MODEL` | `ornith-35b` | 主要模型失敗時的備援；跟主要模型相同就不備援 |
...
| 動作 | 實測 | 備註 |
|---|---|---|
| 菜單照片辨識 | 約 20 秒 | eval 4 張菜單共 77.4 秒；召回 100%、價格 99.2%（`3cda8c1`） |
| 偏好解析 | **1 毫秒** | 純關鍵字比對 |
...
（[src/main.py:590-593](src/main.py#L590-L593)）。實測金鑰無效時回覆只要
...
2. **這裡會等約 20 秒**——講稿要填滿這段：
   「它會先把照片放大到每個字都夠清楚，再讓視覺模型一次讀完整張菜單，
